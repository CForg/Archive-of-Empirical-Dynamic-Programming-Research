...
C***ALLOWS FOR FORTY PERIODS AND 500 POINTS IN INTERPOLATING
     ...
     *   INTP,PMIN
      WRITE(11,1500) NPER,NPOP,DRAW,DRAW1,TAU,NITER,MAXIT,MAXSTP,TRANS,
     *   INTP,PMIN
      WRITE(12,1500) NPER,NPOP,DRAW,DRAW1,TAU,NITER,MAXIT,MAXSTP,TRANS,
     *   INTP,PMIN
 1500 FORMAT(1x,i3,1x,i5,1x,f7.0,1x,f6.0,
     *       1x,f6.0,1x,i2,1x,i2,1X,I2,1x,A3,1x,I5,1x,F4.0)
C     DRAW1=DRAW
      NPARM=27
      PMIN = 1.0/(10.0**PMIN)
...
*******************
 ...
 1602 FORMAT(5(1X,A8))
      DO 202 J=1,4
       ...
       WRITE(11,1603) 
     *   (NAME(K),K=2*6+5+J*(J-1)/2+1,2*6+5+J*(J+1)/2)
 1603  FORMAT(4(1X,A8))
  202 CONTINUE
...
**************************
 ...
 1605 FORMAT(5I1)
      ...
      WRITE(11,1606)  
     *    (IPARM(K),K=2*6+5+J*(J-1)/2+1,2*6+5+J*(J+1)/2)
 1606  FORMAT(4I1)
  204 CONTINUE
*******************************************************
...
C***SKIP TO THE PROPER PLACE IN THE DATA FILE FOR THIS REPLICATION
 ...
 7777 CONTINUE
      DO 7 I=1,NPOP
      READ(10,1000)
     *  IPPP,TIME(I),STATE(I,1),WAGE(I,1),
     *  EXPER(I,1,1),EXPER(I,1,2),EDUC(I,1),LSCHL(I,1)
       IF(I.eq.1) WRITE(11,1002) IRUN,IPPP,TIME(I)
 1002  FORMAT(' REPLICATION ',I4,' STARTS WITH PERSON ',I4,
     *    ' PERIOD ',I2,/)
       DO 8 T=2,TIME(I)
       READ(10,1001)
     *  STATE(I,T),WAGE(I,T),
     *  EXPER(I,T,1),EXPER(I,T,2),EDUC(I,T),LSCHL(I,T)
    8  CONTINUE
    7  CONTINUE
...
*******************************
      ...
      SIGMA(4) = SQRT(A(4,1)**2 + A(4,2)**2 + A(4,3)**2
     *               +A(4,4)**2)
*****************************************************
...
C***CONSTRUCT THE RANDOM VARIABLES FOR EACH ALTERNATIVE
      DO 122 J=1,DRAW
        ...
        C(J,NPER) = A(3,1)*RNN1+A(3,2)*RNN2
     *             +A(3,3)*RNN3
        B(J,NPER) = A(4,1)*RNN1+A(4,2)*RNN2
     *             +A(4,3)*RNN3+A(4,4)*RNN4
        EU1(J,NPER)=EXP(U1)
        EU2(J,NPER)=EXP(U2)
  122 CONTINUE
****************************************************************
...
C1122 format(' K=',I5,' EMAX=',f10.2,' maxe=',f10.2,' y=',f10.6)
      ...
      XR(1) = 1.0
      XR(2) = SQRT(0.00001 + RMAXV(K) - EV1)
      XR(3) = SQRT(0.00001 + RMAXV(K) - EV2)
      XR(4) = SQRT(0.00001 + RMAXV(K) - CBAR)
      XR(5) = SQRT(0.00001 + RMAXV(K) - VHOME)
...
*******************************************************
     ...
     *        +BETA(2,5)*X2+BETA(2,6)*X2**2)
       ...
       EV2 = W2*EXP(SIGMA(2)**2/2.0)             
       RMAXV(K) = AMAX1(EV1,EV2,CBAR,VHOME)
       XR2 = SQRT(0.00001 + RMAXV(K) - EV1)
       XR3 = SQRT(0.00001 + RMAXV(K) - EV2)
       XR4 = SQRT(0.00001 + RMAXV(K) - CBAR)
       XR5 = SQRT(0.00001 + RMAXV(K) - VHOME)
  ...
  162   CONTINUE
       ...
       IF(STATE(I,T).EQ.2) THEN
        DO 163 J=1,DRAW1
         ...
         CJ    = A(3,1)*RNNL1+A(3,2)*RNNJ2
     *          +A(3,3)*RNNL3
         BJ    = A(4,1)*RNNL1+A(4,2)*RNNJ2
     *          +A(4,3)*RNNL3+A(4,4)*RNNL4
         V1 = W1*EXP(U1J) 
         V3 = CBAR+CJ 
         V4 = VHOME+BJ   
         VMAX=AMAX1(V1,V2,V3,V4)
  ...
  163   CONTINUE  
       ...
       IF(ICOUNT0(K).EQ.1) THEN
        DO 164 J=1,DRAW1
         ...
         CJ  = A(3,1)*RNNL1+A(3,2)*RNNL2
     *        +A(3,3)*RNNL3
         BJ  = A(4,1)*RNNL1+A(4,2)*RNNL2
     *        +A(4,3)*RNNL3+A(4,4)*RNNL4
         V1 = W1*EXP(U1J) 
         V2 = W2*EXP(U2J)
         V3 = CBAR+CJ 
...
C****CONSTRUCT THE RANDOM VARIABLES FOR EACH ALTERNATIVE             
      DO 153 J=1,DRAW
        ...
        U1     = A(1,1)*RNN1
        U2     = A(2,1)*RNN1+A(2,2)*RNN2
        C(J,T) = A(3,1)*RNN1+A(3,2)*RNN2
     *          +A(3,3)*RNN3
        B(J,T) = A(4,1)*RNN1+A(4,2)*RNN2
     *          +A(4,3)*RNN3+A(4,4)*RNN4
        EU1(J,T)=EXP(U1)
        EU2(J,T)=EXP(U2)
  153 CONTINUE
...
C     write(11,1122) K,EMAX(K),RMAXV(K),ARG  
      ...
      YBAR = YBAR + Y(KK)/KKMAX 
      XR(1) = 1.0
      XR(2) = SQRT(0.00001 + RMAXV(K) - EW1 - E1)
      XR(3) = SQRT(0.00001 + RMAXV(K) - EW2 - E2)
      XR(4) = SQRT(0.00001 + RMAXV(K) - E3)
      XR(5) = SQRT(0.00001 + RMAXV(K) - E4)
...
*******************************************************
     ...
     *      +BETA(2,5)*X2+BETA(2,6)*X2**2)
       ...
       XR2 = SQRT(0.00001 + RMAXV(K) - V1)
       XR3 = SQRT(0.00001 + RMAXV(K) - V2)
       XR4 = SQRT(0.00001 + RMAXV(K) - V3)
       XR5 = SQRT(0.00001 + RMAXV(K) - V4)
...
C       WRITE(11,6656) I,T,STATE(I,T),PROBI,DENW
       ...
       ELSE
        IF(NP.EQ.NPARM+1) WRITE(11,6655) I,T,STATE(I,T),PROBI
 6655   FORMAT(' *** WARNING: PERSON',I5,' T=',I2,
     *         ' J=',I2,' PROB=',F16.14)
        RLLFI(I,IP) = RLLFI(I,IP) + LOG(PMIN)
        RLOGLF(IP) = RLOGLF(IP) + LOG(PMIN)
      ENDIF  
...
*************************************************************
 6003 CONTINUE
      IF((ITER.EQ.1).AND.(ISTEP.EQ.0))
     *    SRLOGLF = RLOGLF(NITER+1)
      IF(ISTEP.EQ.0) 
     *    RLOGLF0 = RLOGLF(NITER+1)
...
C        WRITE(11,4001) (CPARM(NP),NP=1,NPARM)
 ...
 4384    FORMAT(/,' STEP ',I2,' WAS BAD')  
         IF(IBAD.EQ.MAXSTP) THEN
           WRITE(11,6666) ITER
 6666      FORMAT(/,' ITER=',I2,' COULD NOT FIND INCREASING',
     *         ' STEP ',/)
           GOTO 9999
         ENDIF
      ENDIF
 ...
 1111 FORMAT(//,' RESULTS AT END OF ITERATION ',I2,//,
     ...
     *  ' TRUE VALUE    STD.ERR.      T-STAT') 
      ...
      DO 91 NP = 1,NPARM
       ...
       IP = IP + 1
       WRITE(11,1091) NAME(NP),CPARM(NP),CPARMT(NP),
     *      CPARMS(NP),STDERR(IP),TSTAT(IP)
...
************************************************************
 ...
 5003  FORMAT(' ITER=',I2,' LF=',f12.6,' OLD LF=',F12.6,
     *   ' % IMPROVE=',F10.8,/)
