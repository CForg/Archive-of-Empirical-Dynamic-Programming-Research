...
begin
  ...
  var kk          : integer;
      ...
      with smp, problems[pbind]^.latent do
       ...
       readnext(pzt);       (* transfer / support change prob s*)
       readnext(pht);       (* prob. of hh change h*)
       kk:=1;
...
var
  ...
  i_draw  : array[sim_range] of real;
   ...
   with cur^, st, bellman[ind], expected[time] do
        begin
        fprob := prob;
        if mmp<>nil then incrmom(mmp^,expected[time],fprob);
                       ...
                       repeat
                           ...
                           repeat with moments[dd] do
                              ...
                              { if outmom AND (dd=Tstart) then
                                 writeln('** ',nr:1,sep,nc:1,sep,ne:1,sep,ng:1,sep,nl:1,sep,dd:3,sep,prop[nl]:8:7,sep,selected[dd]:8:7); }
                              end;
