...
!*******************************
	integer types,ntypes,indi_type(4,4),ltypes
...
1919  format(26(1x,f30.25),4(1x,f26.8),1x,i4,5(1x,i2),2(1x,i4),2(1x,i2))    !Parameter file     
	...
	if(iwhat.eq.5)then
      filetexa ='actualpred_'//trim(heq2) //'e5.tex'      
      filetex ='predict_'//trim(heq2)
     * //'a5.tex'            
	elseif(iwhat.eq.6)then
      filetexa ='actualpred_'//trim(heq2) //'e6.tex'      
      filetex ='predict_'//trim(heq2)
     * //'e6.tex'            
...
!**********************************************************************
      ...
      indi_type(2,4)=2
	if(iwhat.ge.5)then
      indi_type(3:4,1:4)=indi_type(1:2,1:4)
      endif
...
!        spar=2*spar
        ...
        xpari1(1)=spar
        write(1,*) 'Policy Experiment  ', ix        
        xtitlef(ix)="Relax Borrowing Constraint"                
        write(1,*) xtitlef(ix),spar
        x1=func(p0)
 	  spar=xpari(1)        
...
!*******************************************
	...
	bmin(2)=xpari(2)      
      ...
      elseif(iwhat.ge.5)then ! just simulations
      npy=0 !pass parameter directly
	x1=func(p0)

!*********************************************************
!     1 Women have men's labor market environment
!*********************************************************
        ...
        xpari1(3)=xt(2)                
        write(1,*) 'Policy Experiment  ', ix
        xtitlef(ix)="Women have men's  arrival rates"
        write(1,*) xtitlef(ix), (xpari1(i),i=1,3)
        x1=func(p0)
        xlu(2)=xpari(1)
        xlo(2)=xpari(2)        
        xt(2)=xpari(3)       
        ...
        xtitlef(ix)="Women have men's  arrival rates
     *   and UI"
        write(1,*) xtitlef(ix), (xpari1(i),i=1,4)
        x1=func(p0)
        xlu(2)=xpari(1)
        xlo(2)=xpari(2)        
        xt(2)=xpari(3)       
        ...
        ix=ix+1	
        indi_type(2,1:4)=indi_type(1,1:4)

        xpari(1)=xm_types(2,1)
        xpari(2)=xm_types(2,2)
        xpari(3)=std(2)        
        xm_types(2,1)=xm_types(1,1)
        xm_types(2,2)=xm_types(1,2)
	std(2)=std(1)
        xpari1(1)=xm_types(2,1)
        xpari1(2)=xm_types(2,2)
        xpari1(3)=std(2)        

        write(1,*) 'Policy Experiment  ', ix
        xtitlef(ix)="Women have men's wage offer distribution"
        write(1,*) xtitlef(ix), (xpari1(i),i=1,3)
        x1=func(p0)
        xm_types(2,1)=xpari(1)
        xm_types(2,2)=xpari(2)
	std(2)=xpari(3)  
        ...
        xtitlef(ix)="Women have men's wage offer distribution
     *   and arrival rates"
        write(1,*) xtitlef(ix), (xpari1(i),i=1,6)
        x1=func(p0)
        xm_types(2,1)=xpari(1)
        xm_types(2,2)=xpari(2)
	std(2)=xpari(3)      
        ...
        xpari(5)=xlo(2)        
        xpari(6)=xt(2)
        xpari(7)=slei_types(2,1)
        xpari(8)=slei_types(2,2)
        ...
        xlo(2)=xlo(1)
        xt(2)=xt(1)
        slei_types(2,1)=slei_types(1,1)
        slei_types(2,2)=slei_types(1,2)
        xpari1(1)=xm_types(2,1)
        xpari1(2)=xm_types(2,2)
        xpari1(3)=std(2)
        xpari1(4)=xlu(2)        
        xpari1(5)=xlo(2)        
        xpari1(6)=xt(2)       
	xpari1(7)=slei_types(2,1)
        xpari1(8)=slei_types(2,2)

            
        write(1,*) 'Policy Experiment  ', ix
        xtitlef(ix)="Women have men's wage offer distribution
     *   arrival rates, and leisure parameters"
        write(1,*) xtitlef(ix), (xpari1(i),i=1,8)
        x1=func(p0)
        xm_types(2,1)=xpari(1)
        xm_types(2,2)=xpari(2)
	std(2)=xpari(3)      
        ...
        xlo(2)=xpari(5)        
        xt(2)=xpari(6)    
	slei_types(2,1)=xpari(7)
	slei_types(2,2)=xpari(8)


        ix=ix+1	
        fin=ix                
!	Default types	
      indi_type(4,1:4)=indi_type(3,1:4)
	...
	xpari1(7)=slei_types(2,1)
        ...
        write(1,*) 'Policy Experiment  ', ix
        xtitlef(ix)="Women have men's wage offer distribution
     *   arrival rates, and leisure parameters and UI"
        write(1,*) xtitlef(ix), (xpari1(i),i=1,9)
        x1=func(p0)
        xm_types(2,1)=xpari(1)
        xm_types(2,2)=xpari(2)
	std(2)=xpari(3)      
...
!*	Input Arguments
	...
	parameter(na=101,nw=101,nc=251,np=29,
!	parameter(na=51,nw=51,nc=251,nmparam = 19,np=19,      
     *	tbig=8,nmobs=48,maxmom=20000,
     *    npeople=140000)            
!     *	nn=33305,nmquest=1069)                  !octubre 2017      0-1 hijos 
!     *	nn=35997 ,nmquest=1077)                  !octubre 2017      2-+ hijos     
!     *	nn= 7374,nmquest=211)                  !marzo 2018      0-1 hijos          con prestaciones
!!     *	nn=27145  ,nmquest=916)                  !marzo 2018      0-1 hijos          sin prestaciones         
...
!*	Integration
      ...
      real*8	ev(0:nw,0:nw,na),vuu,vee!,u11,u10,u01,u0u0,u0u1,u1u0
...
!     Heterogeneity      
      integer types,ntypes,indi_type(4,4),iskip,types1,ltypes
...
!     Monthly interest rate and discount factor      
     ...
     *  ne.xm_types1(i,indi_type(i,types)))iskip=1
      if(slei_types(i,indi_type(i+2,types)).
     * ne.slei_types1(i,indi_type(i+2,types)))iskip=1
      enddo
      ...
      do i=1,2
        ...
        slei(i)=slei_types(i,indi_type(i+2,types))
      enddo   
...
4416  format(i2)         
	...
	if(iwhat.eq.5)then
      filetexer='exercise' // ixlabel // '_'// trim(heq2)// 'e5.tex'
	elseif(iwhat.eq.6)then
      filetexer='exercise' // ixlabel // '_'// trim(heq2)// 'e6.tex'
	endif
...
1919  format(26(1x,f30.25),4(1x,f26.8),1x,i4,5(1x,i2),2(1x,i4),2(1x,i2))    
...
1351  return
      ...
      include 'severalsub.f'      

      include 'des_fit.f'
      
      include 'standevhet.f'      
