...
C***ALLOWS FOR FORTY PERIODS AND 500 POINTS IN INTERPOLATING
 ...
 1500 FORMAT(1x,i3,1x,i5,1x,f7.0,1x,f6.0,
     *       1x,f6.2,1x,i2,1x,i2,1X,I2,1x,A3,1x,I5,1x,F4.0)
C     DRAW1=DRAW
      NPARM=27
...
C***DRAW THE RANDOM VARIABLES FOR SIMULATING THE DP SOLUTION
      ...
      DO 9 T=1,NPER
       ...
       C(J,T) = A(3,1)*RV(1)+A(3,2)*RV(2)
     *            +A(3,3)*RV(3)
       B(J,T) = A(4,1)*RV(1)+A(4,2)*RV(2)
     *            +A(4,3)*RV(3)+A(4,4)*RV(4)
       EU1(J,T)=EXP(U1)
       EU2(J,T)=EXP(U2)
...
C     WRITE(11,1191) EV1,EV2,CBAR,VHOME
 1191 format(' V1=',f12.5,' V2=',f12.5,' V3=',f12.5,' V4=',F12.5,
     *       ' ME=',f12.5)                                          
      IF(RMAXV(K,NPER).GT.EMAX(K,NPER))
     *  EMAX(K,NPER) = RMAXV(K,NPER) 
...
C     Y(KK) = EMAX(K,NPER)
      ...
      XR(1) = 1.0
      XR(2) = SQRT(0.00001 + RMAXV(K,NPER) - EV1)
      XR(3) = SQRT(0.00001 + RMAXV(K,NPER) - EV2)
      XR(4) = SQRT(0.00001 + RMAXV(K,NPER) - CBAR)
      XR(5) = SQRT(0.00001 + RMAXV(K,NPER) - VHOME)
...
C      IF(RMAXV(K,NPER).EQ.VHOME) DUMMY3 = 1.0
       XR2 = SQRT(0.00001 + RMAXV(K,NPER) - EV1)
       XR3 = SQRT(0.00001 + RMAXV(K,NPER) - EV2)
       XR4 = SQRT(0.00001 + RMAXV(K,NPER) - CBAR)
       XR5 = SQRT(0.00001 + RMAXV(K,NPER) - VHOME)
...
C      WRITE(11,6554) KK,Y(KK),XGAMA,YBAR,SSE,SST
 ...
 6555 FORMAT(' SSE=',f12.0,' SST=',F13.0,' RSQ=',F7.5,' YBAR=',f8.1,
     *     ' INTP=',I5)
...
C****INITIALIZE THE EXPECTED MAX TO ZERO
  ...
  550 CONTINUE
     ...
     *      +BETA(2,5)*X2+BETA(2,6)*X2**2)
      ...
      IF(E.LE.19)     
     *   E3 = CBAR + DELTA*EMAX(FSTATE(T+1,X1+1,X2+1,E-8,2),T+1)
...
C     Y(KK) = EMAX(K,T)
      ...
      XR(1) = 1.0
      XR(2) = SQRT(0.00001 + RMAXV(K,T) - EW1 - E1)
      XR(3) = SQRT(0.00001 + RMAXV(K,T) - EW2 - E2)
      XR(4) = SQRT(0.00001 + RMAXV(K,T) - E3)
      XR(5) = SQRT(0.00001 + RMAXV(K,T) - E4)
...
C      IF(RMAXV(K,T).EQ.V4) DUMMY3 = 1.0
       XR2 = SQRT(0.00001 + RMAXV(K,T) - V1)
       XR3 = SQRT(0.00001 + RMAXV(K,T) - V2)
       XR4 = SQRT(0.00001 + RMAXV(K,T) - V3)
       XR5 = SQRT(0.00001 + RMAXV(K,T) - V4)
       XR6 = (RMAXV(K,T) - V1)
       XR7 = (RMAXV(K,T) - V2)
C      XR8 = (RMAXV(K,T) - V3)
       XR8 = (RMAXV(K,T) - V4)
...
C***END OF LOOP OVER PEOPLE
 ...
 3001 format(' CONDITIONAL MEANS AT END OF LIFE :',/,
     *       '   X1 = ',F8.4,' X2=',F8.4,' E=',F8.4,/)                
      do 71 t=1,nper
...
***********************************************************
 ...
 3002 format(/,' UNCONDITIONAL MEANS AT END OF LIFE :',/,
     *       '   X1 = ',F8.4,' X2=',F8.4,' E=',F8.4,/)                
