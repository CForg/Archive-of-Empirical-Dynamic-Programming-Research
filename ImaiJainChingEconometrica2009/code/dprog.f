...
C ===============================================================
            write (500,*) '--- distribution ----'
            write (500,*) 'beta1 beta2 sigmaw = ', beta1, beta2, sigmaw
           end if   
          do iw = 1, niw
               fsum = 0
                do iwf = 1, niw
...
C ----------------------------------------------------------------
	     ...
	     do isim = 1, nsim
	       ...
	       write (500,'(1(I5,1X),6(F9.5,1X))') isim
     +		 ,sh(isim,1),sh(isim,2),val1,val2,max(val1,val2)
...
C ------------------------------------------------------------------
         open (unit = 670, file = 'vsim.out')
           ...
           do iw1 = 1, niw
             ...
             do iwf = 1, niw
               ff = dlog(wgrid(iwf))
     +             -beta1-beta2*dlog(wgrid(iw1))
               ff = dlog(wgrid(iwf))+ff*ff/(2.0*sigmaw*sigmaw)
               ffm(iwf) = dexp(-ff)
               fsum = fsum+ffm(iwf)
             end do
               fwrite = beta1+beta2*dlog(wgrid(iw1))
             ...
             do iwf = 1, niw
               ...
               emax1 = 0.0D0
             do iwf = 1, niw
               emax1 = emax1+vsim(iwf)*ffm(iwf)
             end do
             write (670,'(I5,1X,3(F10.6,1X))')
     +       iw1, wgrid(iw1),emax1, fwrite
